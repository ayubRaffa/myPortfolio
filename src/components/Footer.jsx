...
const Footer = ({ App }) => {
    ...
    const [contentState, setContentState] = useState('get in touch')
    const [heightState, setheightState] = useState(180)
            ...
            <div ref={footerContainer} className='absolute overflow-visible bottom-8 left-0 right-0 text-center border-t border-gray-500 bg-navbar' >
                ...
                <div className="relative flex justify-between ">
                    ...
                    <div className='flex flex-row justify-start w-full gap-4 p-2 flex-nowrap'>
                        <span
                            className={`cursor-pointer hover:text-white ${contentState==='get in touch' && 'text-white'}`} 
                            onClick={() => { setContentState('get in touch'); setheightState(180) }}>
                                get in touch
                        </span>
                        <span 
                            className={`cursor-pointer hover:text-white ${contentState==='about my website' && 'text-white'}`} 
                            onClick={() => { setContentState('about my website'); setheightState(180) }}>
                                about my website
                        </span>
                        {/*     <span className='cursor-pointer hover:text-white' onClick={() => setContentState("what's next")}>what's next</span>*/} 
                    </div>
                ...
                <div className="flex flex-col items-center justify-start gap-2  overflow-hidden " ref={footer}>
                    <h2 className="">{Content[contentState]['h2']}</h2>
                    <p className="font-sans text-sm ">{Content[contentState]['p']}</p>
                    <span>
                        {Content[contentState]['span'] !== undefined && Content[contentState]['span']}
                    </span>
                    <a href={Content[contentState]['a']['link']} target='_blank' rel='noreferrer' className='underline  lg:w-auto'>
                        {Content[contentState]['a']['text']}
                    </a>
                </div>

            </div >
            <div className="absolute bottom-0 right-0 pb-2 bg-navbar left-0 text-center h-8"><span>Handcrafted by me <FaCopyright /> 2023</span></div>
        </div >
    )
}
